//
//  UpdateApplicantRestrictionViewController.swift
//  Bridge
//
...
/// 지원 제한 수정 VC
final class UpdateApplicantRestrictionViewController: BaseViewController {
    // MARK: - UI
    ...
    // MARK: - Property
    private let viewModel: UpdateApplicantRestrictionViewModel

    // MARK: - Init
    init(viewModel: UpdateApplicantRestrictionViewModel) {
    ...
    // MARK: - Bind
    override func bind() {
        let input = UpdateApplicantRestrictionViewModel.Input(
        ...
        output.selectedRestrictions
            .drive(onNext: { [weak self] restrictions in
                ...
                restrictions.forEach { restriction in
                    switch restriction {
                    case "학생": self.studentButton.isSelected = true
                    case "취준생": self.jobSeekerButton.isSelected = true
                    case "현직자": self.currentEmployeeButton.isSelected = true
                    case "제한없음": self.noRestrictionButton.isSelected = true
                    default: print("Error")
                    }
                }
...
extension UpdateApplicantRestrictionViewController {
