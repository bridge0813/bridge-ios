//
//  UpdateProjectDescriptionViewController.swift
//  Bridge
//
...
/// 모집글의 제목과 소개를 수정하는 VC
final class UpdateProjectDescriptionViewController: BaseViewController {
    // MARK: - UI
    ...
    // MARK: - Property
    private let viewModel: UpdateProjectDescriptionViewModel
    
    // MARK: - Init
    init(viewModel: UpdateProjectDescriptionViewModel) {
    ...
    // MARK: - Bind
    override func bind() {
        let input = UpdateProjectDescriptionViewModel.Input(
        ...
        // 이미 작성된 소개
        output.existingDescription
            .drive(onNext: { [weak self] description in
                guard let self else { return }
                self.descriptionTextView.text = description
                self.descriptionTextView.textColor = BridgeColor.gray01
            })
            .disposed(by: disposeBag)
