//
//  UpdateMemberFieldViewController.swift
//  Bridge
//
...
/// 모집하는 팀원의 분야를 수정하는 VC
final class UpdateMemberFieldViewController: BaseViewController {
    // MARK: - UI
    ...
    // MARK: - Property
    private let viewModel: UpdateMemberFieldViewModel

    // MARK: - Init
    init(viewModel: UpdateMemberFieldViewModel) {
    ...
    // MARK: - Binding
    override func bind() {
        let input = UpdateMemberFieldViewModel.Input(
        ...
        output.selectedFields
            .drive(onNext: { [weak self] fields in
                ...
                self.setFieldView.updateButtonStateToMultipleSelections(fields)
            })
